...
function get_player_stats($object,$stat_type='stats') {
	...
	foreach( $object->{'team-sport-content'}[0]->{'league-content'}[0]->{'season-content'}[0]->{'team-content'}[0]->{'player-content'} as $player ) {
		//42932 is Tulo's player ID
		if ( substr( $player->{'player'}->{'id'}, -6 ) == ':42932' ) {
			if ($stat_type == 'stats') {
		...
		if (count($playerchunk) > 2) {
			...
			$player_string = '<div class="playerdata">';
				...
				$player_string .= '<div class="playerhomediv">';
					...
					$player_string .= '<div class="playerhome player-right"><a href="http://stats.denverpost.com/baseball/mlb-players.aspx?page=/data/mlb/players/player42932.html" title="Troy Tulowitzki full statistics">Full Stats &raquo;</a></div>';
					$player_string .= '<div class="clear"></div>';
				$player_string .= '</div>';
			$player_string .= '</div>';
